...
class StochasticFractalSearch():
    ...
    *************************************************************************
    ...
    def batch_fitness(self, points):
        ...
        result = np.array([self.fitness_callback(point) for point in points])
    ...
    def diffusion_process(self, source):
        ...
        new_points = [source.reshape((1, -1))]
        for i in range(num_diffusion):
            # consider which walks should be selected
            sigma = (np.log(self.generation) / self.generation) * (abs((source - self.best_point)))  # eqn (13)
            if rand() < self.walk:
                # simple walk, gaussian around best
                generated_point = normal(loc=self.best_point, scale=sigma, size=(1, dimension)) + (rand() * self.best_point - rand() * source)  # eqn (11),a mistake in the code replacing uniform and normal distributions
            else:
